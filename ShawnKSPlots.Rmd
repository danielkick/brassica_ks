...
ggplot()+
  geom_density(aes(x = sim_ingroup_dist), color = 'blue')+
  geom_density(aes(x = sim_outgroup_dist), color = 'red')+
  geom_density(aes(x = sim_outgroup_dist_wo_ory), color = 'orange')
...
temp2 <- M %>% select(-Group) %>% filter(Species == 'Orychrophragmus_violaceae') %>% rename(Dist = Species)
