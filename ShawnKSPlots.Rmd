...
# remove the (sometimes present) "-\D+$"
# ks_sp$clean <- unlist(map(strsplit(ks_sp$ks_sp, split = '-'), function(e){e[1]}))
...
```{r}
# Simulate the distribution prior to species divergence
# Combine two or more  distributions
...
prep_ecdf_ks <- function(
  ...
  #              #"#67a9cf",
  #              "#1c9099")
  #   downsample_dist = FALSE # force ecdfs to have the same length
  # }
  ...
  # depends on equal length
  #graph.df <- data.frame(data1, data2) %>% gather(Condition, Value, 1:2)
  # makes this not depend on equal values
  graph.df <- rbind(
    data.frame(
...
data.frame(KS = test_arr_diff) %>% 
  ...
  #   ((KS > ci_hdi$CI_low) & (KS < ci_hdi$CI_high)) ~ T,
  #   !((KS > ci_hdi$CI_low) & (KS < ci_hdi$CI_high)) ~F,
  # )) %>% 
  ggplot()+
...
# sp_list <- c('Brassica_tournefortii', 
#              'Erucastrum_virgatum', 
#              'Raphanus_raphanistrum',
#              'Raffenaldia_primuloides' )
...
leaf_nodes <- df$node[!(df$node %in% intersect(df$parent, df$node))]
...
temp <- pr_df[pr_df$parent == target_node, ] %>% 
  select(-parent) %>% 
  gather(key = 'child', value = 'pr') %>% 
  na.omit()
...
pr_df[(pr_df$parent %in% c(1,2,75,74)), c("parent", "1", "2", "3", "74", "75")]
...
pr_df_update = pr_df
pr_df_update[, br.len_cols] = 0



sum(pr_df[, c("1", "2")], na.rm = TRUE)
...
# pr matrix each row is a node, each column is the precent of each node that
pr_mat <- matrix(
  nrow = max(df$node), 
  ncol = max(df$node))
...
```{r}
# set up df to hold species mix at node
sp_percents = data.frame('node' = seq(1, max(tree_df$parent, tree_df$node)))
...
#FIXME
for(i in seq(1, 55)){#nrow(tree_df))){
  i_label  = unlist(tree_df[tree_df$node == i, 'label'])
  i_length = unlist(tree_df[tree_df$node == i, 'branch.length'])
  
  if(!(i_label %in% c('', '1.000000'))){
    sp_percents[(sp_percents$node == i), i_label] <- i_length
  }
  
  
  tree_df[tree_df$node == i, ]
...
tree_df %>% 
  select(-label) %>% 
  spread(key = parent, value = branch.length)
...
tree_df %>% filter(label != '1.000000') %>% select(-branch.length, -parent)



get_sp_lengths <- function(df = tree_df, sp_leaf){
  sp_path <- leaf_to_root(df = df, leaf = sp_leaf)
  sp_path[1, 'branch.length'] <- 1
  sp_path <- sp_path[, c('from', 'branch.length')]
  names(sp_path) <- c('node', sp_leaf)
  return(sp_path)
}
...
for(sp in unique(sp_matches$tr_sp)){
  sp_percents <- full_join(sp_percents, 
                           get_sp_lengths(df = tree_df, sp_leaf = sp))
}
...
map(list("Brassica_cretica",
         "Caulanthus_amplexicaulis-PatBMAP"), 
    function(e){leaf_to_root(df = tree_df, leaf = e)})


for(e in unique(M$Species)){
  print(e)
  print(leaf_to_root(df = tree_df, leaf = e))
}
...
M[M$Group == 'ingroup', 'Species'] %>% unique()





"Orychrophragmus_violaceae-cdhitest90"

M[M$Group == 'outgroup', 'Species'] %>% unique()
